...
CREATE STORAGE INTEGRATION IF NOT EXISTS SF_S3_INTEGRATION
    ...
    STORAGE_PROVIDER          = 'S3'
    ENABLED                   = TRUE
    STORAGE_AWS_ROLE_ARN      = 'arn:aws:iam::ACCOUNT_ID:role/sf-ingest-prod-snowflake-access'  -- CDK output: SnowflakeRoleArn
    STORAGE_ALLOWED_LOCATIONS = ('s3://sf-ingest-prod-landing-ACCOUNT_ID/salesforce/')
;
...
CREATE NOTIFICATION INTEGRATION IF NOT EXISTS SF_SQS_INTEGRATION
    ...
    DIRECTION          = INBOUND
    AWS_SQS_ARN        = 'arn:aws:sqs:us-east-1:ACCOUNT_ID:sf-ingest-prod-snowpipe-notifications'  -- CDK output: SnowpipeQueueArn
;
